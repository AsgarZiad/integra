...
@lru_cache(maxsize=128)
def compile_expression(expr_str: str):
    """Validate and compile a repeated expression once for faster sampling."""
    return compile(preprocess_expression(expr_str), '<math-expression>', 'eval')
...
def evaluate_function(expr_str: str, x_val: float) -> float:
    ...
    """
    compiled_expr = compile_expression(expr_str)
    ...
    try:
        result = eval(compiled_expr, {"__builtins__": None}, safe_dict)
